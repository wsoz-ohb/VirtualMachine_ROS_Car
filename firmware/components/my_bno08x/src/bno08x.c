...
void softReset(void)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(50));     // 等待50ms以确保复位完成
    while (receivePacket() == 1)
        ;          // 清空接收缓冲区
    vTaskDelay(pdMS_TO_TICKS(50)); // 再次等待50ms
    while (receivePacket() == 1)
        ; // 再次清空缓冲区
}
...
uint8_t readFRSdata(uint16_t recordID, uint8_t startLocation, uint8_t wordsToRead)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(10));                        // 等待响应
    ...
    { // 最多尝试10次
        ...
        vTaskDelay(pdMS_TO_TICKS(5)); // 每次尝试间隔5ms
    }
    return 0; // 读取失败
}
